...
<br><br>
## 1. Projektübersicht
Fachlichkeit des Deutschunterrichts: Literaturvermittlung 1782-1891 (FaDeLive: 1782-1891) ist ein Korpus aus 1022 Texten zu Literaturvermittlung an höheren Schulen in Preußen im 19. Jahrhundert. Es ist die Grundlage des Projektes *"Lesen heißt diese Übung" – und dann? Begriffe und intendierte Praktiken der Vermittlung deutschsprachiger Literatur an höheren Schulen 1796–1890*. 

Die analogen Quellen wurden fotomechanisch gescannt, mit OCR verarbeitet und in Hinblick auf OCR-Fehler gesäubert. Diese Verarbeitungsstufe *TXT (content)* ist die Grundlage für die weitere Prozessierung der Daten. Siehe zur Vorverarbeitung und Verarbeitung des Korpus die Diagramme: <https://doi.org/10.48693/730>

Die Funktionen des Repositorium sind Reproduzierbarkeit, Nachnutzung und Weiterentwicklung. 
...
**Schlagwörter**: Computerlinguistik, Didaktik des Deutschunterrichts, Literaturunterricht, Historische Bildungsforschung, Text Mining, Computational Discourse Analyses
<br><br>
## 2. Datengrundlage
Die Pipeline erwartet ein Korpus in folgendem Format: 
...
- `author_surname` - Nachname der Verafsser, der Verfasserin
...
- `editor_prename` - Vorname des Herausgebers, von dem die Neuauflage herausgegeben wurde oder der die Zeitschrift bzw. den Band herausgegeben hat, in dem der Text veröffentlicht wurde
...
- `title_addition` - ausführlicher Titel der Zeitschriften
- `edition` - Auflagennummer bei mehr als einer Auflage
...
- python anhand von `pyproject.toml`
`pip install .`
<br><br>
### Modul 1: Grundlegende NLP-Pipeline

1. Vorverarbeitung von data/raw/korpus.csv und Erzeugung von Textvarianten TXT (min), TXT (lem), TXT (stop) in `output/processed_corpus`

    - für TXT (min): Säuberung von OCR-Fehlern mit  `resources/ocr_post-correction_dictionary.txt`, Normalisierung mit `resources/replacements.json`

    - für TXT (lem): Lemmatisierung mit dem HanoverTagger und `resources/morphmodel_ger.pgz`

    - für TXT (stop): Entfernung von Stoppwörtern mit `resources/stopwords.txt`

2.  Erzeugung des Vokabulars in `output/vocabular`

    - für `genre`, `textclass`

    - für Intervalle (für 4 Intervalle 1782-1852, 1853-1864, 1865-1876, 1877-1891, für 3 Intervalle 1782-1856, 1857-1872, 1873-1891, für 2 Intervalle 1782-1864, 1865-1891)
...
5.  Erzeugung der Vorverarbeitungsstufe TXT (gen) für die Verarbeitung zu Wort-Vektor-Modellen in `output/preprocessed_corpus`
...
9.  Erzeugung der tfidf-Ranglisten des Vokabulars und der Texte in `tfidf/rank`
...
Die mit `fadelive_v3.toml` erzeugten Ausgaben finden sich in <https://doi.org/10.25625/APN9VH>

Neben den unterschiedlichen Pipeline-Konfiguration gibt es eine allgemeine Pipeline, die angepasst werden kann: `src/fadelive/pipeline`.
<br><br>
#### Start der Pipelines
Start einer konfigurierten Pipeline `fadelive_v3.toml` mit `src/fadelive/pipeline_config.py`:<br> 
`python -m fadelive.pipeline_config --config config/fadelive_v3.toml`

Start der Pipeline `src/fadelive/pipeline`:<br>
...
Mit dem Skript `src/tools_output/single_texts_for_tm.py` werden für das Topic-Modelling alle Dokumente in einer beliebigen Verarbeitungsstufe mit IDs und intervallspezifisch geordnet in `src/tools_output/single_texts_for_tm.py` ausgegeben.
<br><br>
### Modul 3: Skripte zur Verarbeitung der getaggten Ausdrücke und Topics
Mit diesem Modul können kontrollierte Vokabulare und Topicmodelle des Korpus verarbeitet werden, um ihre Vokabulare aufeinander abzubilden.

**Funktionen der Skripte**:

`src/procession_termsets_topics/s01_process_stop_pos_tag.py`:<br>
&nbsp;&nbsp;&nbsp; &bull; Verarbeitung einer semantisch getaggten POS-Liste und Ausgabe einer Pivot-Tabelle in `output/processed_tag`

&nbsp;&nbsp;&nbsp; Voraussetzung in `resources/stop_pos_tag/[Vokabular].csv`:<br>
&nbsp;&nbsp;&nbsp; &bull; semantisch getaggte POS-Liste mit den Spalten `word`, `tag1`, `tag2`, `tag3`

`src/procession_termsets_topics/s02_process_topics.py`:<br> 
&nbsp;&nbsp;&nbsp; &bull; Verarbeitung von Topics in `output/processed_topics`

&nbsp;&nbsp;&nbsp; Voraussetzung in `resources/topic-models/[Modell]`<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &bull; Document-Topics-Distribution-Matrix als `.csv` mit Topics als Spaltenindex und Text-IDs als Zeilenindex<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &bull; Top-100-Word-Topic-Matrix `.csv` mit SpaltenIndex `Word 0` bis `Word 99` und Zeilenindex Topicbezeichnungen

`src/procession_termsets_topics/s03_termset-topics_dtti.py`:<br> 
&nbsp;&nbsp;&nbsp; &bull; Berechnung der Termset-Topic-Text-Verhältnisse in `output/processed_termset/[Termset]/`

&nbsp;&nbsp;&nbsp; Voraussetzung: <br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &bull; kontrolliertes Vokabular als Pivottabelle in `resources/termsets/[Termset].csv`<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &bull; verarbeitete Topics `output/processed_topics`

`s04_process_termset-topics_dtti.py`:<br>
&nbsp;&nbsp;&nbsp; &bull; Verarbeitung der Termset-Topic-Text-Verhältnisse in `output/processed_termset/[Termset]/`

&nbsp;&nbsp;&nbsp; Voraussetzung:<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &bull; berechnete Termset-Topic-Text-Verhältnisse in `output/processed_termset`

**Funktionen der Verarbeitung des getaggten Vokabulars, der Termsets und Topics**: 

Eines der grundlegenden Ziele des übergeordneten Projektes ist die Ermittlung von Ausdrücken, die die Fachlichkeit der Literaturvermittlung im 19. Jahrhundert zu bestimmten. Hierzu wurden zunächst zwei Verfahren separat angewendet und dann kombiniert: qualitatives, semantisches Taggen sowie algorithmische Modellierung von Topics und semantisches Taggen der Topics.

Entsprechend der Entwicklung der Verarbeitung des Korpus gibt es drei experimentelle Stufen, denen auch jeweils eine semantisch getaggte Ausdrucksliste und ein semantisch getaggtes Topic-Modell erzeugt wurde. 
- Taglisten: `resources/stop_pos_tag`
- Topic-Modelle: `resources/topic-models`

**Exemplarische Anwendung**: 
- eine POS-getaggte Vokabelliste semantisch taggen, indem der POS-Tag-Liste `output/vocabular/vocab_top5000_stop_pos.csv` die Spalten `tag1`, `tag2` und `tag3` hinzugefügt werden, die Ausdrücke mit bis zu drei Kategorien abstrahiert,
- daraus eine Pivot-Tabelle erzeugen,
- anhand der Tag-Statistiken relevante Tags ausgewählt und die Master-Termsets _Begriffe_, _Gegenstände_ und _Praktiken_ gebildet,
- Termsets auf Topics und Texte abbilden. 
<br><br>
### Modul 4: Skripte für Visualisierungen und Erkundung des Korpus in einem GUI-Tool

Mit den beiden GUI-Tools `src/tools_visualisations/gui_corpus-explorer.py` und`src/tools_visualisations/gui_tag-topic-explorer.py` können das Korpus erkundet und einzelne Daten erzeugt werden.

#### Korpus-Explorer
Der Korpusexplorer ermöglicht gängige korpusanalytische Abfragen. Ausgaben aus der Verarbeitung des Korpus mit der Pipeline und mit einem Termset aus `resourcen/termsets` und einem Topic-Modell aus `resourcen/topic-models` sind alle Abfragen möglich. Alle Dateien sind voreingestellt und können geändert werden. 
...
Neben dem Tab *Daten* gibt es vier Kategorien mit Abfragen und Visualisierungen: 
   ...
   ***Ausdrücke***
   ...
   - Frequenz von gesuchter Ausdrücke in Dokumenten (Abfrage)
   ...
   - Wortverlaufskurve zu gesuchten Ausdrücken (Visualisierung)
   ...
   ***Termset***, 
   - Streudiagramm eines Termsets anhand des Wort-Vektor-Modells mit dem Dimensionsreduktionsalgorithmus _UMAP_ und dem hierarchischen Clusteralgorithmus von _sklearn_ (Visualisierung)<br>
       Parameter:
        - Clusteranzahl
        - Tags als Marker im Plot indizieren
   - Wortwolke eines Termsets erstellen (Visualisierung)
   - Dendrogramme eines Termsets anhand des Wort-Vektor-Modells mit dem hierarchischen Clusteralgorithmus von _sklearn_ (Visualisierung)<br>
       Parameter:
         - Clusteranzahl
...
#### Tag-Topic-Explorer
Der Tag-Topic-Explorer visualisiert die Topics diachron und die Tag-Topic-Verhältnisse sowohl synchron als auch diachron. Für die Abfragen werden die Verarbeitungen aus Modul 3 benötigt. Die Tag-Topic-Verhältnisse ermöglichen eine auf das jeweilige Termset relativierte Abfrage der der Topics. Alle Dateien sind voreingestellt und können geändert werden. 

`gui_tag-topic-explorer.py` benötigt
- Termset als Pivot-Tabelle: `resources\termsets\Termset_Begriffe_2.3.csv`
- Top-100-Word-Topics-Matrix: `resources\topic-models\topics_v3\fadelive_mallet_stop_topic_words_100_words_tag.csv`
...
Neben dem Tab *Daten* gibt es zwei Kategorien Abfragen und Visualisierungen: 
   ...
   ***Tag-Topic-Exploration***
   ...
   - Tendenzkurven zur Entwicklung der Topics anhand der relativ zum Termset und zu den Topics Top-[30]-Texten (Visualisierung)
